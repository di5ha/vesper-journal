...
async def run_test():
    ...
    try:
        ...
        # -> Type 'today' into the sidebar search input (index 233), wait for debounce, then click the first search result (index 237).
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div/div/div/aside/div[2]/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('today')
