...
async def run_test():
    ...
    try:
        ...
        # -> Fill the email field with the test username and password, then click 'Sign in'.
        ...
        # -> Type the query 'zzzxqy_not_a_real_topic_123' into the sidebar search input and wait for results, then extract page content to check for 'No results'.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div/div/div/aside/div[2]/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('zzzxqy_not_a_real_topic_123')
