...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Drift' navigation item to open the Drift page (index 341).
        frame = context.pages[-1]
        # Click element
        ...
        frame = context.pages[-1]
        await expect(frame.locator('xpath=//button[contains(@class,"theme-pill") and contains(@class,"active")]').first).to_be_visible(timeout=3000)
        await expect(frame.locator('text=Mood Timeline').first).to_be_visible(timeout=3000)
